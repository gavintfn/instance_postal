...
#
#apt update
/usr/local/sbin/apt install -y software-properties-common
...
/usr/local/sbin/apt install -y ruby2.3 ruby2.3-dev build-essential mariadb-server libmysqlclient-dev rabbitmq-server nodejs git nginx wget nano
...
#add db info
#
wget https://raw.githubusercontent.com/gavintfn/instance_postal/master/build_messages.sql -O /root/build_messages.sql
...
echo "source /root/build_messages.sql;" | mysql -u root -p'YYYMYSQL_ROOT_PASSWORDYYY'
echo 'GRANT ALL PRIVILEGES ON *.* to `postal`@`127.0.0.1`  IDENTIFIED BY "YYYMYSQL_ROOT_PASSWORDYYY";' | mysql -u root -p'YYYMYSQL_ROOT_PASSWORDYYY'
echo 'GRANT ALL PRIVILEGES ON *.* to `postal`@`localhost`  IDENTIFIED BY "YYYMYSQL_ROOT_PASSWORDYYY";' | mysql -u root -p'YYYMYSQL_ROOT_PASSWORDYYY'
